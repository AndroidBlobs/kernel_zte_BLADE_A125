...
{
	if ((fwupgrade == NULL) && (fwupgrade->func == NULL)) {
...
* Brief: write pramboot to pram and initialize
* Input:
* Output:
...
int fts_fwupg_get_boot_state(struct i2c_client *client, enum FW_STATUS *fw_sts)
{
	...
	if ((fw_sts == NULL) || (upg == NULL) || (upg->func == NULL)) {
...
int fts_fwupg_init(struct fts_ts_data *ts_data)
{
	...
	int func_list_len = sizeof(upgrade_func_list[0]);
	int func_count = 0;

	func_count = sizeof(upgrade_func_list) / func_list_len;
