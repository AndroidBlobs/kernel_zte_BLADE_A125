...
*******************************************************************************/
#define FTS_DRIVER_INFO  "Qualcomm_Ver 1.3 2015-06-11"
...
#define FTS_DBG_EN 1
#if FTS_DBG_EN
#define FTS_DBG(fmt, args...)		pr_info("[FTS]" fmt, ## args)
#else
...
struct fts_ts_data {
	...
	u16 addr;
	bool suspended;
	bool wake_gesture_enter;
	bool wake_gesture_enable;
	char *ts_info;
	u8 *tch_data;
	u32 tch_data_len;
	u8 fw_ver[3];
...
extern  u8 focaltech_chip_id;/*zhangjian add for FW upgrade*/
...
extern void fts_get_upgrade_array_CTPM_FW(void);
